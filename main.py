...
def remove_local_path(path):
    """Safely remove a directory with retries to handle file locks on Windows."""
    import shutil, os
    ...
    def onerror(func, path_arg, exc_info):
        try:
            ...
            print(f"[WARN] Retry delete failed on {path_arg}: {e}")

    if os.path.exists(path):
        for attempt in range(5):
            try:
                shutil.rmtree(path, onerror=onerror)
                print(f"[INFO] Removed directory: {path}")
                break
            except PermissionError as e:
                print(f"[WARN] Cleanup attempt {attempt+1} failed: {e}")
                time.sleep(1)  # wait before retry
        else:
            print(f"[ERROR] Could not remove directory after retries: {path}")
